...
void CncSettingsPanel::renderToolbar() {
    ...
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Export settings as plain text ($N=value format)");
...
void CncSettingsPanel::exportPlainText() {
    ...
    for (const auto& [id, s] : m_settings.getAll()) {
        char line[256];
        if (s.value == std::floor(s.value) && std::abs(s.value) < 1e6f) {
            std::snprintf(line, sizeof(line), "$%d=%d", id, static_cast<int>(s.value));
        } else {
            std::snprintf(line, sizeof(line), "$%d=%.3f", id, static_cast<double>(s.value));
        }
        content += line;
