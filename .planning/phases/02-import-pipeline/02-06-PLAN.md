...
<task type="auto">
  <name>Task 1: Extend LibraryManager with G-code operations and wire ImportQueue to GCodeRepository</name>
  <files>
    src/core/library/library_manager.h
    src/core/library/library_manager.cpp
    src/core/import/import_queue.cpp
...
**Auto-detect filename matching:**
- Strip extension from G-code filename
- Strip common suffixes like "_roughing", "_finishing", "_profile", etc.
- Search ModelRepository::findByName for the base name
- If exactly one match found, return its model ID
- If zero or multiple matches, return nullopt (don't auto-associate)

**ImportQueue G-code persistence (import_queue.cpp):**

In `processTask()`, the Stage 5 (Inserting) already needs G-code handling from Plan 04. Ensure it works:
- For ImportType::GCode:
  - Create GCodeRecord from task data
  - Populate all metadata fields from task.gcodeMetadata
  - Call GCodeRepository::insert(record)
  - Set task.gcodeId = inserted ID
  - Attempt auto-detect: call autoDetectModelMatch logic (or just store standalone)
  - If auto-detect finds a match AND there are no existing groups for that model, do NOT auto-create groups — per user decision, templates are opt-in. Just log the potential match.

**Important:** The G-code auto-detect only attempts to match. It does NOT automatically associate or create groups. Per user decision: "G-code imported standalone goes to a general G-code area; user can drag-and-drop to associate later." The auto-detect result can be stored as a suggestion but not acted upon automatically.
  </action>
  <verify>
Build: `cd /data/DW && cmake --build build --target dw 2>&1 | tail -10` — compiles cleanly.
  </verify>
  <done>
LibraryManager provides G-code CRUD, hierarchy management (operation groups), template application, and auto-detect filename matching. ImportQueue persists G-code records via GCodeRepository with full metadata. Auto-detect is suggestion-only, not auto-action.
  </done>
</task>
