...
must_haves:
  ...
  key_links:
    ...
    - from: "src/core/import/import_queue.cpp"
      ...
      via: "processTask manually creates GCodeLoader for G-code files"
      pattern: "GCodeLoader"
    - from: "src/core/import/import_queue.cpp"
...
- Stage 4 (Parsing): For G-code files, manually create a GCodeLoader instance and call its load/loadFromBuffer method directly — do NOT go through LoaderFactory and cast back. This avoids type safety issues with downcasting.
  ```cpp
  if (task.importType == ImportType::GCode) {
      GCodeLoader gcodeLoader;
      auto result = gcodeLoader.loadFromBuffer(task.fileData);
      if (result.success) {
          task.mesh = std::move(result.mesh);
          task.gcodeMetadata = gcodeLoader.lastMetadata();
      }
  } else {
      // Existing mesh loading via LoaderFactory
      auto result = LoaderFactory::loadFromBuffer(task.extension, task.fileData);
      // ...
  }
  ```
  This is the only correct approach. Do NOT use `static_cast<GCodeLoader*>(loader.get())` on a LoaderFactory result — the factory may return a different loader type, and the cast is unsafe.
- Stage 5 (Inserting):
...
Build: `cd /data/DW && cmake --build build --target dw 2>&1 | tail -10` — compiles cleanly. Run tests: `cd /data/DW/build && ctest --output-on-failure 2>&1 | tail -20` — all existing tests pass.
  ...
  <done>
ImportQueue uses ThreadPool for parallel import. Tasks distributed across N workers (N from Config parallelism tier). G-code files loaded via manual GCodeLoader creation (not LoaderFactory cast) with metadata extraction. Mesh files via existing LoaderFactory path. FileHandler applies copy/move/leave mode after parse. GCodeRepository used for G-code duplicate checks and insertion. BatchSummary accumulates duplicates and errors. Batch complete callback fires when all tasks done.
  </done>
</task>
...
4. G-code files go through manually created GCodeLoader, mesh files through existing loaders
...
- G-code files parsed via direct GCodeLoader creation and inserted into gcode_files table with metadata
- Mesh files continue to work exactly as before via LoaderFactory
