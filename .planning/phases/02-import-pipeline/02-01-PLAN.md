...
2. **File Handling** section:
   ...
   - If copy/move selected, show library directory path with a "Browse" button (or just show path, browse can be deferred)
...
2. Run settings app briefly to verify it doesn't crash on startup (the new tab should appear).
  ...
  <done>
Config has parallelism tier, file handling mode, library directory, and toast toggle fields that persist to INI file. Settings app shows Import tab with combo, radio buttons, and checkbox for all three settings. Both main app and settings app compile without errors.
  </done>
</task>
